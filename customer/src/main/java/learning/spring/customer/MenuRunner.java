...
public class MenuRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        MenuItem item = restTemplate.getForObject(itea + "/menu/{id}", MenuItem.class, 1);
        log.info("菜单上第一个是{}", item);

        String coffee = restTemplate.getForObject(itea + "/menu?name={name}", String.class,
                Collections.singletonMap("name", "Java咖啡"));
        log.info("有Java咖啡么？{}", coffee);

        String menuJson = restTemplate.getForObject(itea + "/menu", String.class);
    ...
    private void getAllMenu() {
        ...
        URI uri = UriComponentsBuilder.fromUriString(itea + "/menu").build().toUri();
